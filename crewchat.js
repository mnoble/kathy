...
$(function() { 
    CrewChat = {
        ...
        start: function start() {
            this.init_events()
            this.name = "Matte"
            this.client = new WebSocket("ws://0.0.0.0:8080?name="+this.name)
        ...
        handle_event: function handle_event(data) {
            switch (data.event) {
            case "joined":
                CrewChat.add_user(data)
                CrewChat.user_event(data, "has entered the room.")
                break
            
            case "disconnected":
                CrewChat.remove_user(data)
                CrewChat.user_event(data, "has disconnected.")
                break
            
            case "slashcommand":
                CrewChat.slash_command(data)
            }
        },
